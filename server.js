...
const supabaseInitPromise = supabaseService.initialize();

// Замените старую функцию initializeSupabase
...
// Замените старую функцию getSupabaseStatus
...
async function callGigaChatAPI(messages, temperature = 0.7, max_tokens = 8000) {
    try {
...
class EnhancedOllamaValidator {
    ...
    async checkAvailability() {
        try {
            ...
            console.log(`✅ Ollama доступен. Модели: ${availableModels.map(m => m.name).join(', ')}`);
            
            // Выбираем лучшие доступные модели из списка
            ...
            // Сопоставляем приоритеты с доступными моделями
            for (const priorityModel of modelPriorities) {
                const found = availableModels.find(m => m.name.includes(priorityModel.split(':')[0]));
                if (found) {
                    this.models.html_expert = found.name;
                    this.models.creative = found.name;
                    this.models.designer = found.name;
                    console.log(`✅ Используется модель: ${found.name}`);
                    break;
                }
            }
            ...
            console.warn('❌ Ollama недоступен для MVP генерации:', error.message);
            this.isAvailable = false;
            return false;
        }
    }
...
Ответь в формате JSON:
{
    ...
    async callOllamaWithRetry(prompt, temperature = 0.3, maxRetries = 2, timeout = 120000) {
...
// ============================================
// УНИВЕРСАЛЬНАЯ ФУНКЦИЯ ВЫЗОВА GIGACHAT API (ЕДИНСТВЕННАЯ ВЕРСИЯ)
// ============================================
...
// ============================================
// REAL_GIGACHAT_MVP_GENERATOR.js - ПОЛНОСТЬЮ ИСПРАВЛЕННЫЙ
// ============================================

class REAL_GIGACHAT_MVP_GENERATOR {
  constructor() {
    console.log('🚀 REAL GigaChat MVP Generator инициализирован');
    this.gigachatAccessToken = null;
    this.gigachatTokenExpiry = null;
    this.credentials = {
      authKey: process.env.GIGACHAT_API_KEY || 'MDE5YjI1YjctMWYyZC03YTU4LTg3NzctZDhhYWQ0MTdkNDk0OjZiN2QyMTFjLTlhYTEtNDg5ZS04Y2UyLTVlOTQ4MTQ5MzE2NQ=='
    };
    this.baseURL = 'https://gigachat.devices.sberbank.ru/api/v1';
    this.model = 'GigaChat';
    this.scope = 'GIGACHAT_API_PERS';
    
    // 🔴 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Не вызываем инициализацию в конструкторе
    this.initialized = false;
    this.initializationPromise = null;
  }

  // ============================================
  // ИНИЦИАЛИЗАЦИЯ GIGACHAT API - БЕЗОПАСНАЯ ВЕРСИЯ
  // ============================================
  ...
  async initialize() {
    ...
    this.initializationPromise = (async () => {
      try {
        console.log('🔑 Получение access token для GigaChat...');
        await this.getAccessToken();
        this.initialized = true;
        console.log('✅ GigaChat API готов к работе');
      } catch (error) {
        console.error('❌ Ошибка инициализации GigaChat:', error.message);
        // 🔴 НЕ бросаем ошибку, чтобы сервер не падал
        this.initialized = false;
        console.log('⚠️  GigaChat API будет работать в тестовом режиме');
      }
    })();
  ...
  // ============================================
  // ПОЛУЧЕНИЕ ACCESS TOKEN - ИСПРАВЛЕННАЯ ВЕРСИЯ
  // ============================================
  ...
  async getAccessToken() {
    try {
      ...
      console.log(`📊 Длина ключа: ${this.credentials.authKey.length}`);
      console.log(`🔑 Base64 ключ: ${this.credentials.authKey.substring(0, 30)}...`);
      ...
      const response = await axios({
        ...
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
          'Accept': 'application/json',
          'Authorization': `Basic ${this.credentials.authKey}`,
          'RqUID': RqUID
        },
        httpsAgent: new https.Agent({
          rejectUnauthorized: false,
      ...
      if (response.status === 200 && response.data?.access_token) {
        ...
        this.gigachatTokenExpiry = new Date(Date.now() + (response.data.expires_in * 1000));
      ...
      // 🔴 ВОЗВРАЩАЕМ ТЕСТОВЫЙ ТОКЕН ДЛЯ РАЗРАБОТКИ
      console.log('⚠️  Использую тестовый токен для разработки');
      this.gigachatAccessToken = 'dummy-gigachat-token-for-testing-only-' + uuidv4();
      this.gigachatTokenExpiry = new Date(Date.now() + 3600000); // 1 час
  ...
  // ============================================
  // ОСНОВНОЙ МЕТОД ГЕНЕРАЦИИ - БЕЗОПАСНАЯ ВЕРСИЯ
  // ============================================

  async generateUltimateMVP(userPrompt) {
    console.log('🚀 НАЧИНАЕМ РЕАЛЬНУЮ ГЕНЕРАЦИЮ ЧЕРЕЗ GIGACHAT');
    console.log('📝 Запрос пользователя:', userPrompt.substring(0, 100) + '...');
    ...
    try {
      ...
      // Если инициализация еще в процессе, ждем
      if (this.initializationPromise) {
      ...
      if (!this.gigachatAccessToken || 
          (this.gigachatTokenExpiry && Date.now() >= this.gigachatTokenExpiry.getTime())) {
        console.log('🔄 Токен отсутствует или истек, получаем новый...');
        await this.getAccessToken();
      }

      // Проверяем, реальный ли токен
      const isRealToken = this.gigachatAccessToken && 
                         !this.gigachatAccessToken.includes('dummy-gigachat-token');
      
      if (!isRealToken) {
        console.log('⚠️  Используется тестовый режим GigaChat');
        return this.generateLocalMVP(userPrompt);
      }

      // Создаем промпт
      const optimizedPrompt = this.createOptimizedPrompt(userPrompt);
      console.log('⚙️ Оптимизированный промпт создан');

      // Вызываем GigaChat API
      console.log('📡 Отправляем запрос к GigaChat API...');
      const generatedCode = await this.callGigaChatAPI(optimizedPrompt);
      console.log(`✅ Код получен (${generatedCode.length} символов)`);

      // Извлекаем чистый HTML
      const cleanHTML = this.extractCompleteHTML(generatedCode);
      
      // Валидируем результат
      this.validateGeneratedHTML(cleanHTML);

      // Возвращаем результат
      return {
        success: true,
        html: cleanHTML,
        analysis: this.generateAnalysis(userPrompt, cleanHTML),
        design: 'GigaChat-Generated Modern Design',
        architecture: this.generateArchitecture(cleanHTML),
        stats: {
          htmlSize: cleanHTML.length,
          components: this.countComponents(cleanHTML),
          features: this.countFeatures(cleanHTML),
          linesOfCode: cleanHTML.split('\n').length
        },
        message: '✅ MVP успешно сгенерирован через GigaChat API!',
        source: 'GigaChat API',
        timestamp: new Date().toISOString()
      };

    } catch (error) {
      console.error('❌ Ошибка реальной генерации:', error.message);
      
      // 🔴 ИСПРАВЛЕНИЕ: Возвращаем локальную версию вместо ошибки
      return this.generateLocalMVP(userPrompt);
    }
  }

  // ============================================
  // ЛОКАЛЬНАЯ ГЕНЕРАЦИЯ MVP (ЗАПАСНОЙ ВАРИАНТ)
  // ============================================

  generateLocalMVP(userPrompt) {
    console.log('🔄 Использую локальную генерацию MVP...');
    
    const html = this.createSimpleMVP(userPrompt);
    ...
    return {
      ...
      analysis: {
        ...
        appDescription: `Приложение для: ${userPrompt.substring(0, 100)}...`,
        generatedBy: 'Local Fallback Generator',
        features: ['Адаптивный дизайн', 'Интерактивный интерфейс', 'Современный UI']
      },
      design: 'Modern Professional Design',
      stats: {
        htmlSize: html.length,
        linesOfCode: html.split('\n').length
      },
      message: '✅ MVP сгенерирован локально (GigaChat недоступен)',
  ...
  // ============================================
  // ВЫЗОВ GIGACHAT API - БЕЗОПАСНАЯ ВЕРСИЯ
  // ============================================

  async callGigaChatAPI(prompt, maxRetries = 2) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`📡 Вызов GigaChat API (попытка ${attempt}/${maxRetries})...`);
        ...
        if (response.status === 200 && response.data?.choices?.[0]?.message?.content) {
          ...
          console.error('❌ ОШИБКА 402: Требуется оплата/активация подписки');
          throw new Error('Ошибка доступа GigaChat (402 Payment Required)');
        }
        else {
          console.error('❌ Неожиданный ответ от GigaChat API:');
          console.error('Статус:', response.status);
          console.error('Данные:', response.data);
          throw new Error(`GigaChat API вернул статус ${response.status}`);
        }

      } catch (error) {
        console.error(`⚠️ Попытка ${attempt} не удалась:`, error.message);
        
        if (attempt === maxRetries) {
          throw new Error(`Все ${maxRetries} попыток не удались: ${error.message}`);
        }
  ...
  createOptimizedPrompt(userPrompt) {
    return `ТЫ ЭКСПЕРТ ПО СОЗДАНИЮ ВЕБ-ПРИЛОЖЕНИЙ MVP. СОЗДАЙ ПОЛНОСТЬЮ РАБОЧЕЕ ВЕБ-ПРИЛОЖЕНИЕ.

ЗАПРОС ПОЛЬЗОВАТЕЛЯ: "${userPrompt}"

ТРЕБОВАНИЯ К ГЕНЕРАЦИИ:

1. ПОЛНЫЙ РАБОЧИЙ КОД В ОДНОМ HTML ФАЙЛЕ
2. СОВРЕМЕННЫЙ ПРОФЕССИОНАЛЬНЫЙ ДИЗАЙН
3. РЕАЛЬНЫЙ РАБОЧИЙ ФУНКЦИОНАЛ ИЗ ЗАПРОСА
4. LOCALSTORAGE ДЛЯ СОХРАНЕНИЯ ДАННЫХ
5. АДАПТИВНЫЙ ДИЗАЙН ДЛЯ ВСЕХ УСТРОЙСТВ
6. ИНТУИТИВНЫЙ UX/UI
7. АНИМАЦИИ И ИНТЕРАКТИВНОСТЬ
8. ВАЛИДНЫЙ HTML5/CSS3/ES6+

ТЕХНИЧЕСКИЕ ТРЕБОВАНИЯ:
- Использовать Tailwind CSS через CDN для стилей
- Использовать Font Awesome для иконок
- Реализовать весь функционал из запроса
- Добавить комментарии к сложным участкам кода
- Валидация форм если есть
- Уведомления пользователю
- Темная/светлая тема

ВЕРСТКА ДОЛЖНА ВКЛЮЧАТЬ:
1. Современную навигацию
2. Основной контент с функционалом
3. Футер с информацией
4. Модальные окна если нужны
5. Загрузку/сохранение состояния
6. Обработку ошибок

ВАЖНО: НАЧНИ НЕМЕДЛЕННО С <!DOCTYPE html>
СГЕНЕРИРУЙ ПОЛНЫЙ КОД, КОТОРЫЙ МОЖНО СРАЗУ СОХРАНИТЬ КАК .html И ОТКРЫТЬ В БРАУЗЕРЕ.
...
СОЗДАЙ УНИКАЛЬНОЕ ПРИЛОЖЕНИЕ ПОД КОНКРЕТНЫЙ ЗАПРОС ПОЛЬЗОВАТЕЛЯ.

ОТВЕТ:`;
  }

  // ============================================
  // ИЗВЛЕЧЕНИЕ И ВАЛИДАЦИЯ HTML
  // ============================================

  extractCompleteHTML(text) {
    console.log('🔍 Извлекаем HTML из ответа...');
    
    if (!text || text.length < 100) {
      throw new Error('Ответ слишком короткий для MVP');
    }

    // Удаляем возможные markdown обратные кавычки
    let cleanText = text
      .replace(/```html\s*/gi, '')
      .replace(/```\s*/gi, '')
      .replace(/`/g, '')
    ...
    const match = cleanText.match(htmlRegex);
    
    if (match) {
      console.log('✅ Полный HTML документ найден');
      return match[0].trim();
    }

    // Если нет DOCTYPE, но есть html теги
    const htmlWithoutDoctype = /<html[\s\S]*?<\/html>/i;
    const match2 = cleanText.match(htmlWithoutDoctype);
    
    if (match2) {
      console.log('✅ HTML найден (без DOCTYPE), добавляем...');
      return `<!DOCTYPE html>\n${match2[0].trim()}`;
    }

    // Если есть только body
    const bodyOnly = /<body[\s\S]*?<\/body>/i;
    const match3 = cleanText.match(bodyOnly);
    
    if (match3) {
      console.log('✅ Body найден, создаем полный документ...');
      return this.createFullHTMLDocument(match3[0].trim());
    }

    // Если это просто HTML код
    if (cleanText.includes('<div') || cleanText.includes('<section') || cleanText.includes('<form')) {
      console.log('✅ HTML элементы найдены, создаем обертку...');
      return this.createFullHTMLDocument(cleanText);
    }

    // Если ничего не найдено - это ошибка
    throw new Error('Не удалось извлечь HTML из ответа GigaChat');
  }

  createFullHTMLDocument(bodyContent) {
    const appName = bodyContent.match(/<h[1-2][^>]*>([^<]+)<\/h[1-2]>/i);
    const title = appName ? appName[1] : 'MVP Приложение';
...
<body class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log('🚀 MVP Приложение запущено!');
            
            // Проверка localStorage
            if (typeof localStorage !== 'undefined') {
                localStorage.setItem('app_last_load', new Date().toISOString());
        ...
        function showNotification(message, type = 'info') {
            ...
            notification.className = \`fixed top-4 right-4 px-6 py-3 rounded-lg shadow-lg animate-fadeIn \${ 
  ...
  // ============================================
  // ВАЛИДАЦИЯ СГЕНЕРИРОВАННОГО HTML
  // ============================================

  validateGeneratedHTML(html) {
    console.log('🔍 Валидация сгенерированного HTML...');
    
    const validations = [
      {
        test: html.includes('<!DOCTYPE html>'),
        message: '✅ DOCTYPE присутствует'
      },
      {
        test: html.includes('<html') && html.includes('</html>'),
        message: '✅ HTML теги присутствуют'
      },
      {
        test: html.includes('<head') && html.includes('</head>'),
        message: '✅ Head секция присутствует'
      },
      {
        test: html.includes('<body') && html.includes('</body>'),
        message: '✅ Body секция присутствует'
      },
      {
        test: html.includes('<script'),
        message: '✅ JavaScript код присутствует'
      },
      {
        test: html.includes('localStorage'),
        message: '✅ localStorage используется'
      },
      {
        test: html.length > 2000,
        message: `✅ Достаточный размер (${html.length} символов)`
      },
      {
        test: html.includes('function') || html.includes('addEventListener'),
        message: '✅ Интерактивность реализована'
      }
    ];
    ...
    if (failed.length > 0) {
      console.warn('⚠️ Некоторые валидации не пройдены:');
      failed.forEach(f => console.warn(`  - ${f.message.replace('✅ ', '❌ ')}`));
    } else {
  ...
  // ============================================
  // СОЗДАНИЕ ПРОСТОГО MVP
  // ============================================

  createSimpleMVP(businessIdea) {
    const title = businessIdea.split(' ').slice(0, 3).join(' ') || 'Business MVP';
    const description = businessIdea.substring(0, 150);
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .feature-card { background: white; padding: 2rem; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
        .cta { padding: 4rem 0; text-align: center; background: #2E75B6; color: white; }
        .footer { padding: 2rem 0; background: #1a202c; color: white; text-align: center; }
    </style>
</head>
<body>
    <header class="header">
        <div class="container">
            <nav style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <div>
                    <a href="#features" style="color: white; margin: 0 1rem; text-decoration: none;">Возможности</a>
                    <a href="#about" style="color: white; margin: 0 1rem; text-decoration: none;">О проекте</a>
                    <a href="#contact" style="color: white; margin: 0 1rem; text-decoration: none;">Контакты</a>
                </div>
            </nav>
        </div>
    </header>
    ...
    <main>
        <section class="hero">
            <div class="container">
                ...
                <button style="margin-top: 2rem; padding: 1rem 2rem; background: #10b981; color: white; border: none; border-radius: 5px; font-size: 1rem; cursor: pointer;">Начать сейчас</button>
            </div>
        </section>
        
        <section class="features" id="features">
            <div class="container">
                <h2 style="text-align: center; margin-bottom: 2rem;">Ключевые возможности</h2>
                <div class="feature-grid">
                    <div class="feature-card">
                        <h3>Инновационное решение</h3>
        ...
        <section class="cta" id="contact">
            <div class="container">
                <h2 style="margin-bottom: 1rem;">Готовы начать?</h2>
                <p style="margin-bottom: 2rem; max-width: 600px; margin-left: auto; margin-right: auto;">Присоединяйтесь к сотням успешных предпринимателей</p>
                <button style="padding: 1rem 2rem; background: white; color: #2E75B6; border: none; border-radius: 5px; font-size: 1rem; font-weight: bold; cursor: pointer;">Попробовать бесплатно</button>
            </div>
        </section>
    </main>
    ...
    <footer class="footer">
        <div class="container">
            ...
            <p style="margin-top: 1rem; font-size: 0.9rem;">MVP сгенерировано Strategix AI Pro</p>
        </div>
    </footer>
    ...
    <script>
        ...
        document.querySelectorAll('button').forEach(button => {
            button.addEventListener('click', function() {
                alert('Спасибо за интерес! Функционал в разработке.');
            });
        });
  ...
  generateAnalysis(userPrompt, html) {
    const features = this.extractFeaturesFromHTML(html);
    
    return {
      appName: this.extractAppName(html),
      appDescription: `Приложение для: ${userPrompt.substring(0, 100)}...`,
      appType: this.determineAppType(userPrompt),
      targetAudience: 'пользователи веб-приложений',
      ...
      generatedBy: 'GigaChat API',
      timestamp: new Date().toISOString(),
      estimatedComplexity: this.estimateComplexity(features.length, html.length)
    };
  }

  generateArchitecture(html) {
    return {
      components: this.extractComponents(html),
      dataFlow: "UI → JavaScript Logic → localStorage → UI",
      interactions: this.extractInteractions(html),
      stateManagement: "LocalStorage + JavaScript State Objects",
      externalDependencies: this.extractDependencies(html),
      performance: {
        estimatedLoadTime: "1-2s",
        bundleSize: `${Math.round(html.length / 1024)} KB`,
        complexity: "medium"
      }
    };
  }
  ...
  extractFeaturesFromHTML(html) {
    ...
    if (htmlLower.includes('todo') || htmlLower.includes('задач') || htmlLower.includes('task')) {
      features.push('Управление задачами', 'Добавление/удаление', 'Статусы задач');
    }
    
    if (htmlLower.includes('form') && (htmlLower.includes('submit') || htmlLower.includes('input'))) {
      features.push('Формы ввода', 'Валидация данных', 'Обработка отправки');
    }
    
    if (htmlLower.includes('localstorage') || htmlLower.includes('save') || htmlLower.includes('load')) {
      features.push('Локальное сохранение', 'Восстановление состояния');
    }
    
    if (htmlLower.includes('dark') && htmlLower.includes('theme')) {
      features.push('Переключение темы');
    }
    
    if (htmlLower.includes('notification') || htmlLower.includes('alert') || htmlLower.includes('toast')) {
      features.push('Уведомления пользователя');
    }
    
    if (htmlLower.includes('search') || htmlLower.includes('filter') || htmlLower.includes('sort')) {
      features.push('Поиск и фильтрация');
    }
    
    features.push('Адаптивный дизайн', 'Интерактивный интерфейс', 'Современный UI');
  ...
  extractAppName(html) {
    ...
    return 'MVP Приложение';
  }

  determineAppType(prompt) {
    const promptLower = prompt.toLowerCase();
    
    if (promptLower.includes('todo') || promptLower.includes('задач')) return 'task_manager';
    if (promptLower.includes('календарь') || promptLower.includes('calendar')) return 'calendar_app';
    if (promptLower.includes('чат') || promptLower.includes('chat')) return 'chat_app';
    if (promptLower.includes('дашборд') || promptLower.includes('dashboard')) return 'dashboard';
    if (promptLower.includes('магазин') || promptLower.includes('shop')) return 'ecommerce';
    
    return 'web_application';
  }

  countComponents(html) {
    const tags = ['div', 'section', 'form', 'button', 'input', 'nav', 'header', 'footer'];
    let count = 0;
  ...
  countFeatures(html) {
    const featureIndicators = [
      'function', 'addEventListener', 'onclick', 'onsubmit',
      'localStorage', 'JSON.parse', 'JSON.stringify',
      'classList', 'innerHTML', 'appendChild'
    ];
    
    return featureIndicators.filter(indicator => 
  ...
  extractComponents(html) {
    ...
    if (html.includes('nav') || html.includes('header')) {
      components.push({ name: "Navigation", type: "layout", purpose: "Навигация по приложению" });
    }
    
    if (html.includes('form')) {
      components.push({ name: "Form", type: "input", purpose: "Ввод данных пользователем" });
    }
    
    if (html.includes('button')) {
      components.push({ name: "Buttons", type: "interaction", purpose: "Действия пользователя" });
    }
    
    if (html.includes('localStorage')) {
      components.push({ name: "StorageManager", type: "data", purpose: "Управление локальными данными" });
    }
    
    return components;
  }

  extractInteractions(html) {
    const interactions = ['click', 'submit', 'input', 'load'];
    const found = interactions.filter(interaction => 
      html.toLowerCase().includes(interaction.toLowerCase())
    );
    
    return found.length > 0 ? found : ['click', 'load'];
  }

  extractDependencies(html) {
    const deps = [];
    
    if (html.includes('tailwindcss.com')) deps.push('Tailwind CSS');
    if (html.includes('font-awesome')) deps.push('Font Awesome');
    if (html.includes('googleapis.com')) deps.push('Google Fonts');
    
    return deps.length > 0 ? deps : ['Tailwind CSS (предположительно)'];
  }

  estimateComplexity(featuresCount, htmlSize) {
    if (featuresCount > 8 && htmlSize > 5000) return "high";
    if (featuresCount > 5 && htmlSize > 3000) return "medium";
    return "low";
  }

  // ============================================
  // МЕТОД ДЛЯ ТЕСТИРОВАНИЯ СВЯЗИ
  // ============================================

  async testConnection() {
    try {
      console.log('🧪 Тестирование подключения к GigaChat API...');
      ...
      const isRealToken = token && !token.includes('dummy-gigachat-token');
      
      return {
        success: true,
        message: isRealToken ? 'GigaChat API доступен и работает' : 'Используется тестовый режим',
        token_type: isRealToken ? 'real' : 'test',
        token_length: token?.length,
        real_api_available: isRealToken
      };
...
module.exports = { REAL_GIGACHAT_MVP_GENERATOR };

// Для совместимости со старым кодом
class ULTIMATE_MVP_GENERATOR extends REAL_GIGACHAT_MVP_GENERATOR {
  constructor() {
    super();
    console.log('🚀 Ultimate MVP Generator 2.0 инициализирован с REAL GigaChat API');
  }
}

module.exports.ULTIMATE_MVP_GENERATOR = ULTIMATE_MVP_GENERATOR;

if (typeof window !== 'undefined') {
  window.REAL_GIGACHAT_MVP_GENERATOR = REAL_GIGACHAT_MVP_GENERATOR;
  window.ULTIMATE_MVP_GENERATOR = ULTIMATE_MVP_GENERATOR;
  console.log('🚀 REAL_GIGACHAT_MVP_GENERATOR готов к использованию в браузере');
}
...
3. Ключевые пункты для обсуждения с юристом: список из 3-5 спорных моментов`;
    ...
    try {
        ...
        // 🔴 УВЕЛИЧИВАЕМ ТАЙМАУТ и уменьшаем объем данных
...
КОРОТКИЙ ОБЗОР ДОКУМЕНТА (первые 1500 символов):
${content.substring(0, 1500)}

Проанализируй:
1. Полнота документа (все ли обязательные разделы присутствуют)
2. Качество юридических формулировок
3. Заполнение всех полей данными

Верни ТОКО JSON без дополнительного текста:
{
    "completeness_score": 0-100,
    ...
    "summary": "краткий вывод на русском"
        ...
        // 🔴 ИСПОЛЬЗУЕМ callOllamaWithRetry с увеличенным таймаутом
        const response = await validator.callOllamaWithRetry(validationPrompt, 0.2, 3, 90000); // 90 секунд
        ...
        // 🔴 ЕСЛИ OLLAMA НЕ РАБОТАЕТ - используем только эвристическую проверку
        console.log('⚠️ Ollama недоступен, использую эвристическую проверку');
        return this.fallbackValidation(content, docType);
...
heuristicValidateLegalDocument(content, docType) {
    ...
    const requiredKeywords = {
        founders_agreement_ru: ['участник', 'учредитель', 'дол', 'вестинг', 'выход'],
        safe_en: ['safe', 'investment', 'valuation', 'cap', 'equity'],
        term_sheet_ru: ['терм', 'инвест', 'оценк', 'преференц', 'акци'],
        shareholders_agreement_ru: ['акционер', 'совет', 'решен', 'голосован'],
        saas_agreement_en: ['license', 'subscription', 'service', 'data', 'warranty']
    };
    
    const keywords = requiredKeywords[docType] || [];
    ...
    if (missingKeywords.length > 0) {
        checks.push({
            ...
            message: `Отсутствуют ключевые термины: ${missingKeywords.join(', ')}`,
            severity: 'medium'
        });
        score -= 15;
    }
    ...
    const requiredSections = {
        founders_agreement_ru: ['преамбула', 'определен', 'ведение', 'ответственность', 'конфиденциальность'],
        safe_en: ['parties', 'investment', 'valuation', 'conversion', 'governing law'],
        term_sheet_ru: ['условия', 'оценка', 'инвестиции', 'права', 'заключение']
    };
    ...
    if (missingSections.length > 0) {
        checks.push({
            ...
            message: `Отсутствуют разделы: ${missingSections.join(', ')}`,
            severity: 'high',
    ...
    // Проверка на наличие заполненных данных
    const placeholderPattern = /\[.*?\]/g;
    const placeholders = content.match(placeholderPattern) || [];
    const unfilledPlaceholders = placeholders.filter(p => p === '[]' || p === '[ ]');
    
    if (unfilledPlaceholders.length > 3) {
        checks.push({
            type: 'unfilled_placeholders',
            message: `Много незаполненных полей: ${unfilledPlaceholders.length}`,
            severity: 'medium'
        });
        score -= 10;
    }
    ...
    if (wordCount < 500) {
        checks.push({
            ...
            message: `Документ слишком короткий: ${wordCount} слов`,
            severity: 'medium'
        });
        score -= 15;
    }
    
    // Проверка на наличие конкретных цифр и дат
    const hasNumbers = /\d+/.test(content);
    if (!hasNumbers) {
        checks.push({
            type: 'no_numbers',
            message: 'Нет конкретных цифр или дат',
            severity: 'low'
        });
        score -= 5;
    }
    
    // Проверка на наличие юридических формулировок
    const legalPhrases = ['в соответствии', 'обязуется', 'права', 'обязанности', 'ответственность'];
    const hasLegalLanguage = legalPhrases.some(phrase => lowerContent.includes(phrase));
    
    if (!hasLegalLanguage) {
        checks.push({
            type: 'informal_language',
            message: 'Недостаточно юридических формулировок',
            severity: 'low'
        });
        score -= 5;
    }
    
    // Убедимся, что оценка в пределах разумного диапазона
    ...
    return {
        ...
        unfilled_placeholders: unfilledPlaceholders.length,
        issues: checks.filter(c => c.severity === 'high').map(c => c.message),
        warnings: checks.filter(c => c.severity === 'medium').map(c => c.message),
        recommendations: [
            ...(missingKeywords.length > 0 ? [`Добавьте термины: ${missingKeywords.join(', ')}`] : []),
            ...(missingSections.length > 0 ? [`Добавьте разделы: ${missingSections.join(', ')}`] : []),
            ...(unfilledPlaceholders.length > 0 ? ['Заполните все поля в квадратных скобках'] : []),
            ...(wordCount < 500 ? ['Расширьте документ, добавьте детали'] : []),
            ...(!hasNumbers ? ['Добавьте конкретные цифры и даты'] : []),
            ...(!hasLegalLanguage ? ['Используйте более формальные юридические формулировки'] : [])
        ]
    };
}

fallbackValidation(content, docType) {
    const heuristicCheck = this.heuristicValidateLegalDocument(content, docType);
    
    return {
        completeness_score: heuristicCheck.score,
        legal_quality_score: 60,
        missing_sections: heuristicCheck.checks.filter(c => c.type === 'missing_sections').map(c => c.sections).flat(),
        requires_lawyer_review: true,
        summary: 'Проверка выполнена без AI (Ollama недоступен)',
        heuristic_check: heuristicCheck,
        overall_score: heuristicCheck.score,
        validated_at: new Date().toISOString()
    };
}
...
// ============================================
// УПРОЩЕННЫЙ ГЕНЕРАТОР ТЕХНИЧЕСКИХ ЗАДАНИЙ
// ============================================
...
class TechnicalSpecGenerator {
    constructor() {
        this.templates = {
            ...
            'mobile_app_ru': {
                name: 'ТЗ для мобильного приложения',
                sections: [
                    'Общее описание',
    ...
    async generateTechnicalSpec(templateId, data) {
        try {
            console.log(`📋 Генерация ТЗ: ${templateId}`);
            
            const template = this.templates[templateId];
            if (!template) {
                throw new Error(`Шаблон ${templateId} не найден. Доступные: ${Object.keys(this.templates).join(', ')}`);
            }

            // Создаем промпт для GigaChat
            const prompt = this.createSpecPrompt(template, data);
            
            // Получаем ответ от GigaChat
            const response = await callGigaChatAPI([
                {
                    role: 'system',
                    content: 'Ты - технический писатель с опытом создания ТЗ. Пиши четко, структурировано, с конкретными техническими деталями.'
                },
                { role: 'user', content: prompt }
            ], 0.4, 4000);

            // Парсим ответ
            const structuredSpec = this.parseSpecResponse(response, template, data);
            ...
            return {
                ...
                type: 'technical_spec',
                template: templateId,
                content: structuredSpec,
                raw_content: response,
                generated_at: new Date().toISOString(),
                estimated_hours: this.calculateHours(data),
            ...
            // Возвращаем простой шаблон в случае ошибки
            return {
                success: false,
                fallback: true,
                id: uuidv4(),
                type: 'technical_spec',
                template: templateId,
                content: this.generateFallbackSpec(templateId, data),
                generated_at: new Date().toISOString(),
                error: error.message
            };
        }
    ...
    createSpecPrompt(template, data) {
        return `Создай техническое задание по следующему шаблону:
...
ИНСТРУКЦИИ:
1. Заполни каждую секцию конкретным содержанием
2. Используй данные из раздела "ДАННЫЕ ПРОЕКТА"
3. Будь максимально конкретным в технических деталях
4. Добавь подразделы где это необходимо
5. Включи примеры и пояснения
6. Используй технический язык, но понятный для заказчика
...
Верни ПОЛНЫЙ текст технического задания.`;
    }

    parseSpecResponse(response, template, data) {
        // Простой парсинг - если есть структура, возвращаем как есть
        if (response.includes('##') || response.includes('# ')) {
            return response;
        }

        // Если нет структуры, создаем ее
        let structured = `# ${data.projectName || 'Техническое задание'}\n\n`;
        structured += `*Сгенерировано: ${new Date().toLocaleDateString('ru-RU')}*\n\n`;
        ...
        for (const line of lines) {
            // Если строка похожа на начало новой секции
            if (currentSection < sections.length && 
                (line.toLowerCase().includes(sections[currentSection].toLowerCase().slice(0, 10)) || 
                 lines.indexOf(line) % 5 === 0)) {
                
                structured += `## ${currentSection + 1}. ${sections[currentSection]}\n\n`;
                currentSection++;
            }
        ...
        // Добавляем пропущенные секции
        while (currentSection < sections.length) {
            structured += `## ${currentSection + 1}. ${sections[currentSection]}\n\n`;
            structured += `[Содержание для раздела "${sections[currentSection]}"]\n\n`;
            currentSection++;
        }
    ...
    calculateHours(data) {
        // Простая оценка трудозатрат
        const baseHours = 80;
        const featureMultiplier = data.features?.length || 1;
        const complexityMultiplier = data.complexity === 'high' ? 1.5 : 
                                   data.complexity === 'medium' ? 1.2 : 1;
        
        return Math.round(baseHours * featureMultiplier * complexityMultiplier);
    }

    generateFallbackSpec(templateId, data) {
        const template = this.templates[templateId] || this.templates.web_app_ru;
        
        let spec = `# ${data.projectName || 'Техническое задание'}\n\n`;
        spec += `*Сгенерировано автоматически: ${new Date().toLocaleDateString('ru-RU')}*\n\n`;
        spec += `*ВНИМАНИЕ: Это базовая версия ТЗ. Рекомендуется доработка специалистом.*\n\n`;

        template.sections.forEach((section, index) => {
            spec += `## ${index + 1}. ${section}\n\n`;
            
            switch(section.toLowerCase()) {
                case 'общее описание':
                    spec += `${data.description || 'Проект по разработке программного обеспечения.'}\n\n`;
                    break;
                case 'цели и задачи':
                    spec += `1. Создать работающий продукт\n`;
                    spec += `2. Обеспечить удобство использования\n`;
                    spec += `3. Реализовать основные функции\n\n`;
                    break;
                case 'функциональные требования':
                    if (data.features && Array.isArray(data.features)) {
                        data.features.forEach((feature, i) => {
                            spec += `${i + 1}. ${feature}\n`;
                        });
                    } else {
                        spec += `- Регистрация и авторизация пользователей\n`;
                        spec += `- Основной функционал продукта\n`;
                        spec += `- Административная панель\n`;
                    }
                    spec += '\n';
                    break;
                case 'технический стек':
                    if (data.techStack) {
                        Object.entries(data.techStack).forEach(([category, techs]) => {
                            spec += `**${category}:** ${Array.isArray(techs) ? techs.join(', ') : techs}\n`;
                        });
                    } else {
                        spec += `- Frontend: React/Vue.js\n`;
                        spec += `- Backend: Node.js/Python\n`;
                        spec += `- Database: PostgreSQL/MySQL\n`;
                        spec += `- Infrastructure: Docker, AWS/Azure\n`;
                    }
                    spec += '\n';
                    break;
                case 'сроки разработки':
                    spec += `Ориентировочные сроки: ${data.timeline || '3-6 месяцев'}\n\n`;
                    break;
                case 'бюджет':
                    spec += `Ориентировочный бюджет: ${data.budget || 'уточняется'}\n\n`;
                    break;
                default:
                    spec += `[Содержание раздела "${section}"]\n\n`;
            }
        });
    ...
    async generateAllSpecs(data) {
        const specs = {};
        
        for (const [templateId, template] of Object.entries(this.templates)) {
            try {
                specs[templateId] = await this.generateTechnicalSpec(templateId, data);
            } catch (error) {
                specs[templateId] = {
                    success: false,
                    error: error.message,
                    fallback: this.generateFallbackSpec(templateId, data)
                };
            }
        }
...
// ============================================
// ИНИЦИАЛИЗАЦИЯ КЛАССОВ (ИСПРАВЛЕННАЯ ВЕРСИЯ)
// ============================================
...
// ВАЖНО: Добавьте генератор ТЗ
const techSpecGenerator = new TechnicalSpecGenerator();

// ОСНОВНЫЕ ГЕНЕРАТОРЫ MVP
const mvpGenerator = new REAL_GIGACHAT_MVP_GENERATOR();

// АЛИАСЫ ДЛЯ СОВМЕСТИМОСТИ СО ВСЕМИ ВАРИАНТАМИ ИСПОЛЬЗОВАНИЯ
const ultimateMVPGenerator = mvpGenerator;
const cleanMVPGenerator = mvpGenerator;
const ultimateOllamaMVPGenerator = mvpGenerator; // ЭТОТ ТЕПЕРЬ БУДЕТ РАБОТАТЬ

console.log('🚀 Все генераторы инициализированы, включая GigaChat MVP Generator');
console.log('🔧 ultimateOllamaMVPGenerator теперь доступен');

// Ждем инициализацию в начале запуска сервера
supabaseInitPromise.then(connected => {
    if (connected) {
        console.log('✅ SupabaseService успешно инициализирован');
    } else {
        console.warn('⚠️  SupabaseService не подключен. Функции базы данных будут ограничены');
    }
}).catch(error => {
...
// 10. Аутентификация - Логин (ОБНОВЛЕННЫЙ И ИСПРАВЛЕННЫЙ)
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        if (email === 'test@strategix.ai' && password === 'password123') {
            ...
            const userId = uuidv4();
            const features = getFeaturesBySubscription('pro');
            
            // 🔴 СОЗДАЕМ ТОКЕН ПРАВИЛЬНО:
        ...
        // 🔴 ИСПРАВЛЕНИЕ: Правильное создание токена
        const token = jwt.sign(
            { 
                id: user.id, 
                email: user.email,
...
// ============================================
// РЕАЛЬНАЯ РЕГИСТРАЦИЯ ДЛЯ ВСЕХ ТИПОВ ПОЧТЫ
// ============================================

// Замените эндпоинт /api/auth/register на этот:
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        // 📌 ВАЖНО: Создаем пользователя БЕЗ email верификации
        const userId = uuidv4();
        ...
        await supabaseService.createUserSettings(userId);
        
        // 🔥 Создаем JWT токен для немедленного входа
        const token = jwt.sign(
            { 
                id: user.id, 
                email: user.email,
        ...
        console.log(`✅ Пользователь зарегистрирован (без email верификации): ${user.email}`);
        ...
        res.json({
            ...
            user: {
                ...
                email: user.email,
                name: user.name,
                is_verified: true, // 🔥 Подтвержден
                subscription: user.subscription || 'free'
            },
            token: token, // 🔥 Сразу выдаем токен
            token_type: 'Bearer',
        ...
        if (error.message.includes('уже существует')) {
            ...
            statusCode = 400;
        } else if (error.message.includes('формат email')) {
...
// ============================================
// УНИВЕРСАЛЬНАЯ ВЕРИФИКАЦИЯ EMAIL ДЛЯ ВСЕХ ПОЧТ
// ============================================
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        const user = await supabaseService.createUser(email, password, name);
        
        // 3. Отправляем email верификации (в фоновом режиме, не блокируя ответ)
        sendVerificationEmail(email, name).catch(console.error);
...
app.get('/api/auth/verify-email', async (req, res) => {
    try {
        ...
        // Выполняем верификацию через SupabaseService
        const user = await supabaseService.verifyEmail(token);
                
        // Отправляем HTML страницу с подтверждением
        res.send(`
            <!DOCTYPE html>
            <html>
            <head>
                <title>Email Verified - Strategix AI Pro</title>
                <style>
                    body {
            ...
            <body>
                <div class="container">
                    ...
                    <p>Ваш email <strong>${user.email}</strong> был успешно верифицирован.</p>
                    <p>Добро пожаловать в Strategix AI Pro, ${user.name}!</p>
                    
                    <div class="user-info">
                        <p><strong>Статус аккаунта:</strong> ✅ Активен</p>
                        <p><strong>Подтвержден:</strong> ${new Date().toLocaleDateString('ru-RU')}</p>
                        <p><strong>Доступные функции:</strong> Все функции платформы</p>
                    </div>
                    
                    <div class="actions">
                        <a href="/" class="btn">На главную</a>
                        <a href="/api/auth/login" class="btn btn-primary">Войти в систему</a>
                    </div>
                    
                    <script>
                        // Сохраняем токен в localStorage для автоматического входа
                        localStorage.setItem('token', '${token}');
                        console.log('Токен сохранен для автоматического входа');
                    </script>
                </div>
            </body>
            </html>
...
// Эндпоинт для отладки пользователей
app.get('/api/debug/users', async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            error: error.message,
            supabase_connected: supabaseService.connected
        });
    }
});

// Эндпоинт для повторной отправки письма верификации
app.post('/api/auth/resend-verification', authenticateToken, async (req, res) => {
    try {
        ...
        // Отправляем email
        const emailSent = await supabaseService.sendVerificationEmail(
            email, 
            newToken, 
            user.name
        );
        ...
        res.json({
            ...
            message: emailSent ? 
                ...
                'Письмо не отправлено (проверьте настройки email)',
...
app.post('/api/auth/resend-verification-universal', async (req, res) => {
    try {
        const { email } = req.body;
        ...
        console.log(`📧 Запрос повторной отправки верификации для: ${email}`);
        ...
        // Проверяем настройку email транспорта
        if (!supabaseService.mailTransporter) {
            console.warn('⚠️ Email транспорт не настроен, пропускаем отправку');
            
            return res.json({
                success: true,
                message: 'Пользователь найден, но email не отправлен (транспорт не настроен)',
                email_sent: false,
                verification_token: newToken,
                verification_link: `http://localhost:${PORT}/api/auth/verify-email-universal?token=${newToken}`,
                email_setup_required: true,
                instructions: [
                    'Для отправки верификационных писем настройте email в .env файле:',
                    'EMAIL_HOST=smtp.your-email-server.com',
                    'EMAIL_PORT=587',
                    'EMAIL_USER=your-email@example.com',
                    'EMAIL_PASSWORD=your-password',
                    'EMAIL_FROM=noreply@strategix.ai'
                ]
            });
        }
        ...
        // Отправляем email
        try {
            await supabaseService.sendVerificationEmail(email, newToken, user.name);
            ...
            return res.json({
                ...
                note: 'Проверьте папку "Спам" если не видите письмо'
            });
            ...
            return res.json({
                success: true,
                message: 'Пользователь обновлен, но email не отправлен',
                email_sent: false,
                verification_token: newToken,
                verification_link: `http://localhost:${PORT}/api/auth/verify-email-universal?token=${newToken}`,
...
// ============================================
// ТЕСТ ВСЕХ ТИПОВ ПОЧТЫ
// ============================================

app.post('/api/test/email-validation', (req, res) => {
    try {
        const { email } = req.body;
        ...
        // 🔴 УНИВЕРСАЛЬНЫЙ РЕГЕКС ДЛЯ ВСЕХ ПОЧТОВЫХ СЕРВИСОВ
        ...
        if (emailLower.includes('@gmail.com')) emailService = 'gmail';
        else if (emailLower.includes('@mail.ru') || emailLower.includes('@inbox.ru') || emailLower.includes('@list.ru') || emailLower.includes('@bk.ru')) emailService = 'mail.ru';
        else if (emailLower.includes('@yandex.ru') || emailLower.includes('@ya.ru')) emailService = 'yandex';
        else if (emailLower.includes('@outlook.com') || emailLower.includes('@hotmail.com') || emailLower.includes('@live.com')) emailService = 'microsoft';
        else if (emailLower.includes('@yahoo.com')) emailService = 'yahoo';
        else if (emailLower.includes('@icloud.com')) emailService = 'apple';
        else if (emailLower.includes('@protonmail.com') || emailLower.includes('@proton.me')) emailService = 'protonmail';
        else if (emailLower.includes('@tutanota.com') || emailLower.includes('@tuta.io')) emailService = 'tutanota';
        else {
            // Определяем по домену
            const domain = email.split('@')[1];
            emailService = `custom (${domain})`;
        }
        ...
        res.json({
            ...
            validation: {
                ...
                recommended_validation: 'passes_all_checks'
            },
            registration_compatible: true,
            notes: 'Этот email может быть использован для регистрации'
        });
...
// ============================================
// ТЕСТ EMAIL ТРАНСПОРТА
// ============================================

app.get('/api/test/email-transport', async (req, res) => {
    try {
        if (!supabaseService.mailTransporter) {
            return res.json({
                success: false,
                message: 'Email транспорт не настроен',
                setup_required: true,
                env_variables: {
                    EMAIL_HOST: process.env.EMAIL_HOST || 'не настроен',
                    EMAIL_USER: process.env.EMAIL_USER ? 'настроен' : 'не настроен',
                    EMAIL_PASSWORD: process.env.EMAIL_PASSWORD ? 'настроен' : 'не настроен'
                },
                instructions: [
                    'Добавьте в .env файл:',
        ...
        const verifyResult = await supabaseService.mailTransporter.verify();
        ...
        const testEmail = {
            ...
            subject: 'Тест email транспорта - Strategix AI Pro',
            text: 'Это тестовое письмо для проверки работы email системы.',
            html: '<p>Это тестовое письмо для проверки работы email системы.</p>'
        };
        
        const sendResult = await supabaseService.mailTransporter.sendMail(testEmail);
        ...
        res.json({
            ...
            transport_config: {
                host: supabaseService.mailTransporter.options.host,
                port: supabaseService.mailTransporter.options.port,
                secure: supabaseService.mailTransporter.options.secure,
                auth_user: supabaseService.mailTransporter.options.auth.user.substring(0, 5) + '...'
            },
            message: 'Email транспорт работает корректно'
        });
        
    } catch (error) {
        console.error('❌ Тест email транспорта не пройден:', error);
        ...
        res.json({
            ...
            troubleshooting: [
                ...
                'Для mail.ru используйте порт 465 с secure: true',
                'Для gmail используйте порт 587 с secure: false',
                'Включите доступ "ненадежным приложениям" в настройках почты'
            ]
        });
    }
});
...
app.post('/api/chat/save', authenticateToken, async (req, res) => {
    try {
        ...
        // Сохраняем каждое сообщение
        for (const message of messages) {
            await supabaseService.saveChatMessage(userId, session_id, message);
        }
...
app.post('/api/documents/save', authenticateToken, async (req, res) => {
    try {
        ...
        await supabaseService.saveGeneratedDocument(userId, document_data);
...
// ============================================
// КОМПЛЕКСНАЯ ГЕНЕРАЦИЯ MVP + ТЗ
// ============================================

// 38. Генерация Ultimate MVP с Ollama (исправленная версия)
app.post('/api/generate-ultimate-mvp', async (req, res) => {
  try {
    const { 
      businessIdea, 
      designStyle = 'modern_professional',
      colorScheme = 'blue_gradient',
      complexity = 'enterprise',
      features = ['responsive', 'interactive', 'forms', 'animations'],
      title = 'AI Generated MVP'
    } = req.body;
    
    if (!businessIdea || businessIdea.trim().length < 30) {
      return res.status(400).json({
        success: false,
        error: 'Business idea must be at least 30 characters long'
      });
    }

    console.log(`🚀 Ultimate MVP генерация: ${title}`);
    
    const result = await ultimateOllamaMVPGenerator.generateUltimateMVP(businessIdea, { 
      designStyle,
      colorScheme,
      complexity,
      features,
      title,
      description: `${businessIdea.substring(0, 150)}...`,
      keywords: 'mvp, startup, business, ai generated'
    });
    ...
    res.status(500).json({
      success: false,
      error: 'MVP generation failed: ' + error.message,
      suggestion: 'Убедитесь что Ollama запущен: ollama serve',
      fallback_available: true,
      fallback_endpoint: '/api/generate-html-prototype'
    });
  }
});

// После эндпоинтов для юридических документов добавьте:

// ============================================
// ЭНДПОИНТЫ ДЛЯ ГЕНЕРАЦИИ И ЭКСПОРТА
// ============================================

// 1. Генерация ТЗ
app.post('/api/tech-spec/generate', authenticateToken, async (req, res) => {
    try {
        ...
        if (!template) {
            return res.status(400).json({
                success: false,
                error: 'Укажите шаблон ТЗ. Доступные: web_app_ru, mobile_app_ru, api_service'
            });
        }
        
        const techSpecGenerator = new TechnicalSpecGenerator();
        const result = await techSpecGenerator.generateTechnicalSpec(template, data || {});
...
// 2. Получение шаблонов ТЗ
app.get('/api/tech-spec/templates', authenticateToken, (req, res) => {
    try {
        const techSpecGenerator = new TechnicalSpecGenerator();
        const templates = techSpecGenerator.getTemplates();
...
// 3. УНИВЕРСАЛЬНЫЙ ЭКСПОРТ
app.post('/api/export/universal', authenticateToken, async (req, res) => {
    try {
        const { data, format, filename, options } = req.body;
        
        if (!data || !format) {
            return res.status(400).json({
                success: false,
                error: 'Данные и формат обязательны. Форматы: excel, pdf, txt, json, pptx'
            });
        }
        
        const result = await simpleExporter.exportData(data, format, {
...
// 4. Быстрый экспорт (без авторизации для тестирования)
app.post('/api/export/quick', async (req, res) => {
    try {
        ...
        if (!data || !format) {
            return res.status(400).json({
                success: false,
                error: 'Данные и формат обязательны'
            });
        }
        
        const result = await simpleExporter.exportData(data, format, {
            filename: `quick_export_${Date.now()}`
        });
        ...
        res.status(500).json({
            success: false,
            error: 'Быстрый экспорт не удался: ' + error.message
        });
    }
});
...
app.get('/api/export/test', async (req, res) => {
    try {
        ...
        const testData = {
            ...
            description: 'Это тестовый проект для проверки экспорта',
            metrics: {
                revenue: 1000000,
                profit: 250000,
                growth: 15.5,
            ...
            analysis: {
                conclusion: 'Проект перспективный',
                recommendations: [
                    'Увеличить маркетинговый бюджет',
                    'Расширить команду',
                    'Запустить новые фичи'
                ]
            },
            timeline: '6 месяцев',
            budget: '2,500,000 руб.'
        };
        
        // Предлагаем скачать тестовый файл
        const result = await simpleExporter.exportData(testData, 'excel', {
...
// 40. Улучшенный статус с поддержкой юридических документов
app.get('/api/status/full', async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            platform: 'Strategix AI Pro v8.0.0 Enhanced',
            version: '8.0.0',
            timestamp: new Date().toISOString(),
            uptime: process.uptime(),
...
// ============================================
// ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ ТЗ
// ============================================
...
function createMonthlyBreakdown(totalCost, months) {
...
function getMonthlyDeliverables(month, totalMonths) {
  const deliverables = [
...
function createTeamStructure(team) {
  const structure = {
...
function identifyDependencies(phases) {
  const dependencies = [];
...
// 41. Универсальная генерация документов
app.post('/api/documents/generate-universal', authenticateToken, async (req, res) => {
    try {
        const { document_category, document_type, data, options } = req.body;
        
        if (!document_category || !document_type || !data) {
            return res.status(400).json({
                success: false,
                error: 'Категория, тип документа и данные обязательны'
            });
        }
        ...
        switch(document_category) {
            ...
            case 'tech_spec':
                result = await techSpecGenerator.generateTechnicalSpec(
                    document_type,
                    data,
                    options || {}
                );
            ...
            default:
                return res.status(400).json({
                    success: false,
                    error: `Неизвестная категория: ${document_category}. Доступные: legal, tech_spec, business`
                });
        }
        ...
        console.error('❌ Ошибка универсальной генерации:', error);
        res.status(500).json({
            success: false,
            error: 'Ошибка генерации: ' + error.message
        });
...
// Эндпоинт для получения доступных шаблонов ТЗ
app.get('/api/technical-spec/templates', authenticateToken, (req, res) => {
    try {
        const templates = techSpecGenerator.getAllTemplates();
        ...
        const templateList = Object.entries(templates).map(([key, template]) => ({
            ...
            sections: template.validationRules || []
        }));
        ...
        console.error('❌ Ошибка получения шаблонов ТЗ:', error);
        res.status(500).json({
            success: false,
            error: 'Ошибка получения шаблонов: ' + error.message
        });
...
// Эндпоинт для проверки структуры ТЗ
app.post('/api/technical-spec/validate', authenticateToken, async (req, res) => {
    try {
        const { spec_type, data } = req.body;
        
        if (!spec_type || !data) {
            return res.status(400).json({
                success: false,
                error: 'Тип ТЗ и данные обязательны'
            });
        }
        
        // Создаем тестовую версию ТЗ для валидации
        const testSpec = await techSpecGenerator.generateTechnicalSpec(
            spec_type,
            data,
        ...
        // Проверяем качество
        const validation = await techSpecGenerator.validateTechnicalSpec(
            testSpec.content?.full_document || JSON.stringify(testSpec, null, 2),
            spec_type,
            data
        );
        ...
        res.json({
            success: true,
            validation: validation,
            estimated_hours: testSpec.estimated_hours,
            complexity: testSpec.complexity_level,
            missing_sections: validation.missing_sections || []
        });
        
    } catch (error) {
        console.error('❌ Ошибка валидации ТЗ:', error);
        res.status(500).json({
            success: false,
...
// Эндпоинт для комплексной технической документации
app.post('/api/technical/generate-comprehensive', authenticateToken, async (req, res) => {
    try {
        const { 
            project_name, 
            project_description, 
        ...
        if (!project_name || !project_description) {
            return res.status(400).json({
                success: false,
                error: 'Название проекта и описание обязательны'
            });
        }
        
        console.log(`🔧 Комплексная генерация тех. документации: ${project_name}`);
        
        // Определяем тип проекта
        function detectProjectType(description) {
    const desc = description.toLowerCase();
    if (desc.includes('web') || desc.includes('веб') || desc.includes('сайт')) return 'web_app';
    if (desc.includes('mobile') || desc.includes('мобильн') || desc.includes('приложение')) return 'mobile_app';
    if (desc.includes('api') || desc.includes('сервис') || desc.includes('микросервис')) return 'api_spec_en'; // Добавьте эту строку
    if (desc.includes('saas') || desc.includes('платформ') || desc.includes('облачн')) return 'saas_platform';
    return 'web_app';
}
        ...
        const specData = {
            ...
            target_audience: requirements?.audience || 'Пользователи веб-приложений',
            tech_stack: tech_stack || {
                frontend: ['React', 'TypeScript', 'Tailwind CSS'],
                backend: ['Node.js', 'Express', 'PostgreSQL'],
                infrastructure: ['Docker', 'AWS', 'CI/CD']
            },
            timeline: timeline || '6 месяцев',
            team_structure: team || [
            ...
            budget: requirements?.budget || '3,000,000 руб.',
            success_metrics: requirements?.metrics || [
                'Время загрузки страницы < 3 секунд',
                'Поддержка 10,000+ одновременных пользователей',
                '99.9% uptime',
                'Конверсия пользователей > 5%'
            ]
        };
        
        // Генерируем ТЗ
        const techSpec = await techSpecGenerator.generateTechnicalSpec(
            projectType === 'web_app' ? 'web_app_ru' : 'saas_platform_ru',
            specData,
            { enhanced_validation: true }
        );
        
        // Генерируем API спецификацию если есть требования
        let apiSpec = null;
        if (requirements?.api_endpoints) {
            apiSpec = {
                endpoints: requirements.api_endpoints,
                authentication: 'JWT Bearer tokens',
                rate_limiting: '100 requests/minute per IP',
                error_handling: 'RESTful error responses'
            };
        }
        
        // Генерируем архитектурную схему
        const architecture = {
            layers: [
                { name: 'Presentation Layer', components: ['UI Components', 'Client-side Logic'] },
                { name: 'Business Layer', components: ['Controllers', 'Services', 'Business Logic'] },
                { name: 'Data Access Layer', components: ['Repositories', 'Database Adapters'] },
                { name: 'Infrastructure Layer', components: ['Database', 'Caching', 'File Storage'] }
            ],
            deployment: {
        ...
        res.json({
            ...
            delivery_package: {
                ...
                estimated_delivery: '5-7 рабочих дней'
            }
        });
        
    } catch (error) {
        console.error('❌ Ошибка генерации тех. документации:', error);
        res.status(500).json({
            success: false,
            error: 'Ошибка генерации: ' + error.message
        });
...
// Вспомогательная функция для определения типа проекта
function detectProjectType(description) {
    const desc = description.toLowerCase();
    if (desc.includes('web') || desc.includes('веб') || desc.includes('сайт')) return 'web_app';
    if (desc.includes('mobile') || desc.includes('мобильн') || desc.includes('приложение')) return 'mobile_app';
    if (desc.includes('api') || desc.includes('сервис') || desc.includes('микросервис')) return 'api_service';
    if (desc.includes('saas') || desc.includes('платформ') || desc.includes('облачн')) return 'saas_platform';
    return 'web_app'; // по умолчанию
}
...
app.use('/api/*', (req, res) => {
    res.status(404).json({
        ...
        available_endpoints: [
            ...
            '/api/legal/generate',
            '/api/export/:format',
            '/api/unitmaster-pro/analyze'
        ]
    });
});

// Добавьте этот эндпоинт для тестирования токена
app.get('/api/test-gigachat-token', async (req, res) => {
    try {
        console.log('🔑 Тестирование получения токена GigaChat...');
        
        const token = await getGigaChatTokenReal();
        
        // Проверяем, реальный ли токен или тестовый
        const isRealToken = !token.includes('dummy-gigachat-token');
        
        res.json({
            success: true,
            test: 'giga_chat_token_generation',
            token_received: true,
            token_type: isRealToken ? 'real' : 'test',
        ...
        console.error('❌ Тест токена не пройден:', error);
        res.status(500).json({
            success: false,
            error: 'Тест не пройден: ' + error.message,
            timestamp: new Date().toISOString()
...
// Тестовый вызов API GigaChat
app.get('/api/test-gigachat-api', async (req, res) => {
    try {
        console.log('🧪 Тестовый вызов GigaChat API...');
        
        const response = await callGigaChatAPI([
            { role: 'system', content: 'Ты - помощник для тестирования API. Отвечай кратко.' },
            { role: 'user', content: 'Ответь "API работает" одним предложением.' }
        ], 0.5, 50);
        
        res.json({
            success: true,
            test: 'giga_chat_api_call',
            result: response,
            api_working: true,
            timestamp: new Date().toISOString()
        });
    } catch (error) {
        console.error('❌ Тест API не пройден:', error);
        res.status(500).json({
            success: false,
            error: 'Тест не пройден: ' + error.message,
            api_working: false,
...
// Добавьте этот эндпоинт для диагностики GigaChat
app.get('/api/diagnose/gigachat', async (req, res) => {
    try {
        console.log('🔧 Диагностика GigaChat...');
        
        // 1. Проверка токена
        const token = await getGigaChatTokenReal();
        const tokenValid = token && token.length > 100 && !token.includes('dummy');
        
        // 2. Пробуем разные варианты вызова
        const testResults = [];
        ...
        try {
            ...
            testResults.push({
                name: 'Стандартный с X-Client-ID',
        ...
        res.json({
            success: true,
            diagnosis: {
                token_received: tokenValid,
                token_length: token?.length,
                your_credentials: {
                    client_id: '019b25b7-1f2d-7a58-8777-d8aad417d494',
                    client_secret_masked: '1e250164-217e-45d2-b983-66f3f811ed64',
                    has_package: '1,000,000 токенов до 2027-01-06'
                },
                test_results: testResults,
                recommendations: [
                    'Проверьте в кабинете Sber ID: Scope должен быть GIGACHAT_API_PERS',
                    'Убедитесь что продукт "Активный", а не "Архивный"',
                    'Попробуйте создать новый Authorization Key в кабинете',
...
function createSimpleMVP(businessIdea, options = {}) {
  const title = options.title || 'Business MVP';
  const description = businessIdea.substring(0, 150);
...
function detectBusinessType(text) {
    const lowerText = text.toLowerCase();
    if (lowerText.includes('saas') || lowerText.includes('программ')) return 'saas';
...
💡 УЛУЧШЕННЫЕ МОДУЛИ:
   ...
   ✅ Pitch Deck Generator (10/15 слайдов)
   ✅ Enhanced UnitMaster Pro (industry comparison + readiness score)
   ⚖️ Legal Document Generator (5 типов документов)
...
📋 НОВЫЕ API ЭНДПОИНТЫ:
   ...
   • POST /api/legal/generate (5 типов документов)
   • POST /api/legal/export (DOCX, PDF, TXT, HTML)
   • POST /api/export/:format (универсальный экспорт)
